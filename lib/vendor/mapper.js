...
Mapper.prototype = {
  generate_coords: function() {
    ...
    sys.puts("Room templates is " + this.room_templates.length);
    ...
    while( y < this.ysize ) {
      ...
      sys.puts("Widest with" + widest_width);
      ...
      for(n=y; n < (y + tallest_height); n++) {
        ...
        sys.puts("Arr len: " + arr.length + " N is " + n);
      ...
      if(this.coded_coords[0][0] == ' ') {
        sys.puts("is blank");
      } else {
        sys.puts("is not blank");
      }
      ...
      // sys.puts("Coded coords" + this.coded_coords);
    ...
    _.each(usable_room_exit_pairs, function(exit_pair, exit_index) {
      ...
      sys.puts("Other exit " + other_exit_pairs[0]);
      ...
      _.each(other_exit_pairs, function(other_pair, other_exit_index) {
        ...
        sys.puts("Other pair" + other_pair.length)
        sys.puts("Exit pair" + exit_pair.length)
